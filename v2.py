...
def calculate_morphological_features(cell_roi, nucleus_mask, cytoplasm_mask, pixel_to_um=0.25):
    """Calculate features with robust error handling and physical unit conversion"""
    features = {
        'cell_area': 0,
        'nucleus_area': 0,
        'cytoplasm_area': 0,
    ...
    try:
        ...
        features['nucleus_area_um2'] = features['nucleus_area'] * (pixel_to_um**2)
        features['cytoplasm_area_um2'] = features['cytoplasm_area'] * (pixel_to_um**2)
        features['cell_area_um2'] = features['cell_area'] * (pixel_to_um**2)
        
        features['nc_ratio'] = features['nucleus_area'] / (features['cytoplasm_area'] + 1e-6)
...
def visualize_segmentation_medical(cell_roi, nucleus_mask, cytoplasm_mask, cell_id, nc_ratio):
    """
    Medical visualization with:
    - VIOLET background (cytoplasm)
    - BLACK nucleus (segmented area)
    - WHITE nuclear details
    - YELLOW nucleus outline
    - Cell ID and N:C ratio labels
    """
    ...
    padding = 30
    labeled_vis = np.zeros((vis.shape[0] + padding, vis.shape[1], 3), dtype=np.uint8)
    labeled_vis[:vis.shape[0], :] = vis
    
    # Cell ID (cyan)
    cv2.putText(labeled_vis, f"ID:{cell_id}", (10, 20), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    
    # N:C ratio (yellow)
    cv2.putText(labeled_vis, f"N:C:{nc_ratio:.2f}", (10, vis.shape[0] + 20), 
...
def process_image(frame, model, args):
    ...
    try:
        ...
        for i in range(len(detections)):
            try:
                ...
                if confidence > args.thresh:
                    ...
                    morph_features = calculate_morphological_features(cell_roi, nucleus_mask, cytoplasm_mask, pixel_to_um)
                    
            
                    seg_vis = visualize_segmentation_medical(
                        cell_roi, 
                        nucleus_mask, 
                        cytoplasm_mask, 
                        cell_id, 
                        morph_features['nc_ratio']
                    )
                    ...
                    label = f"ID:{cell_id} {class_name} {confidence*100:.1f}% N:C:{morph_features['nc_ratio']:.2f}"
                    cv2.putText(frame, label, (xmin, ymin-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                    
  
                    area_label = f"N:{morph_features['nucleus_area_um2']:.1f}μm² C:{morph_features['cytoplasm_area_um2']:.1f}μm²"
                    cv2.putText(frame, area_label, (xmin, ymax+15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        if segmented_cells:
            ...
            for i, cell_img in enumerate(segmented_cells):
                ...
                if y + cell_size < output_height and x + cell_size < left_panel_width:
                    ...
                    h_ratio = cell_size / cell_img.shape[0]
                    new_width = int(cell_img.shape[1] * h_ratio)
                    resized = cv2.resize(cell_img, (new_width, cell_size))
                    

                    if x + new_width <= left_panel_width:
                        left_panel[y:y+cell_size, x:x+new_width] = resized
                    else:
                   
                        resized = cv2.resize(cell_img, (cell_size, int(cell_img.shape[0] * cell_size / cell_img.shape[1])))
                        if y + resized.shape[0] <= output_height:
                            left_panel[y:y+resized.shape[0], x:x+cell_size] = resized
            ...
            try:
                ...
                if confidence > args.thresh:
                    ...
                    if (0 <= rxmin < right_panel_width and 0 <= rymin < output_height and
                        ...
                        cell_idx = i % len(features_data) if features_data else -1
                        if 0 <= cell_idx < len(features_data):
                            nc_ratio = features_data[cell_idx].get('nc_ratio', 0)
                            nucleus_area_um2 = features_data[cell_idx].get('nucleus_area_um2', 0)
                            label = f"ID:{i+1} {class_name} {confidence*100:.1f}% N:C:{nc_ratio:.2f}"
                            area_label = f"N:{nucleus_area_um2:.1f}μm²"
                        else:
                        ...
                        if area_label:
                            cv2.putText(right_panel, area_label, (rxmin, rymax+15), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        if total_cells > 0:
            ...
            if not features_df.empty and 'nc_ratio' in features_df.columns:
                avg_nc = features_df['nc_ratio'].mean()
                cv2.putText(right_panel, f"Avg N:C Ratio: {avg_nc:.2f}", 
                           (text_x, text_y + 3*line_height), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    ...
    if args.save_csv:
        try:
            ...
            fieldnames = [
                ...
                'cell_area', 'nucleus_area', 'cytoplasm_area', 'nc_ratio', 
                'perimeter', 'circularity', 'irregularity', 'convexity', 
                'cytoplasm_perimeter', 'cell_area_um2', 'nucleus_area_um2', 
                'cytoplasm_area_um2'
            ]
